import { AlertTriangle, Clock, ArrowRight, CalendarClock, Wrench, Trophy } from 'lucide-react'
...
export default function DashboardListas({ listas, moduloAtivo, navigate }) {
  ...
  const renderMedalha = (index) => {
    if (index === 0) return "bg-gradient-to-br from-yellow-300 to-yellow-500 text-white shadow-md shadow-yellow-500/30 border-none" 
    if (index === 1) return "bg-gradient-to-br from-slate-300 to-slate-400 text-white shadow-md shadow-slate-500/30 border-none" 
    if (index === 2) return "bg-gradient-to-br from-orange-400 to-orange-600 text-white shadow-md shadow-orange-600/30 border-none" 
    return "bg-slate-100 text-slate-400 border border-slate-200" 
  }

  return (
    <div className={`grid grid-cols-1 ${isImpressoras ? 'xl:grid-cols-3' : 'lg:grid-cols-2'} gap-6 w-full`}>
      
      {/* 1. OS ATRASADAS */}
      <div className="bg-white rounded-[2rem] border border-slate-200/80 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full min-h-[320px]">
        ...
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {listas.atrasadas.length === 0 ? <div className="p-8 text-center text-slate-400 text-sm h-full flex items-center justify-center font-bold">Nenhuma OS em atraso! 🎉</div> : listas.atrasadas.map(ch => (
            <div key={ch.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 group">
              {/* 🚀 MIN-W-0 OBRIGATÓRIO PARA O TRUNCATE FUNCIONAR NO FLEXBOX */}
              <div className="flex items-center gap-3 min-w-0 flex-1 pr-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-black text-slate-800 truncate">{ch.equipamento?.nome || 'Equipamento Excluído'}</p>
                  <p className="text-xs font-bold text-slate-400 truncate mt-0.5">{ch.tipo_intervencao} • Previsto para {new Date(ch.data_prevista).toLocaleDateString('pt-BR', {timeZone:'UTC'})}</p>
                </div>
              </div>
              <button onClick={() => navigate(`/${moduloAtivo}/chamados`, { state: { openDetailsId: ch.id } })} className="p-2 bg-white border border-slate-200 hover:border-red-200 hover:bg-red-50 rounded-full transition-all shrink-0 shadow-sm group-hover:shadow"><ArrowRight size={16} className="text-slate-400 hover:text-red-500" /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200/80 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full min-h-[320px]">
        ...
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {listas.proximas.length === 0 ? <div className="p-8 text-center text-slate-400 text-sm h-full flex items-center justify-center font-bold">Nenhum agendamento futuro.</div> : listas.proximas.map(ch => (
            <div key={ch.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 group">
              {/* 🚀 MIN-W-0 OBRIGATÓRIO PARA O TRUNCATE FUNCIONAR NO FLEXBOX */}
              <div className="flex items-center gap-3 min-w-0 flex-1 pr-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-black text-slate-800 truncate">{ch.equipamento?.nome || 'Equipamento Excluído'}</p>
                  <p className="text-xs font-bold text-slate-400 truncate mt-0.5">{ch.tipo_intervencao} • Agendado para {new Date(ch.data_prevista).toLocaleDateString('pt-BR', {timeZone:'UTC'})}</p>
                </div>
              </div>
              <button onClick={() => navigate(`/${moduloAtivo}/chamados`, { state: { openDetailsId: ch.id } })} className="p-2 bg-white border border-slate-200 hover:border-sky-200 hover:bg-sky-50 rounded-full transition-all shrink-0 shadow-sm group-hover:shadow"><ArrowRight size={16} className="text-slate-400 hover:text-sky-500" /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      {isImpressoras && (
        <div className="bg-white rounded-[2rem] border border-slate-200/80 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full min-h-[320px]">
          ...
          <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {listas.top5Cor.length === 0 ? <div className="p-8 text-center text-slate-400 text-sm h-full flex items-center justify-center font-bold">Sem dados de auditoria lançados.</div> : listas.top5Cor.map((item, i) => (
              <div key={item.id} className="p-4 flex items-center justify-between hover:bg-rose-50/30 transition-colors border-b border-slate-50 last:border-0">
                <div className="flex items-center gap-3 min-w-0 flex-1 pr-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black text-slate-800 truncate">{item.usuario_setor}</p>
                    <p className="text-[11px] text-slate-400 font-bold truncate mt-0.5">{item.equipamento?.nome || 'Equipamento Excluído'}</p>
                  </div>
                </div>
