  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-slate-500 font-medium animate-pulse text-sm">Preparando seu ambiente de trabalho...</p>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-slate-100/70 flex flex-col items-center justify-between p-4 md:p-8 animate-in fade-in duration-500 font-sans relative overflow-hidden">
      ...
      <div className="max-w-7xl w-full relative z-10 my-auto py-10">
        <div className="text-center mb-12 md:mb-14">
          <div className="inline-flex items-center justify-center px-4 py-2 bg-white rounded-2xl shadow-sm border border-slate-200/80 mb-6">
        ...
        {erro ? (
          ...
          <div className="bg-white text-slate-600 p-8 rounded-3xl border border-slate-200 flex flex-col items-center justify-center text-center max-w-lg mx-auto shadow-sm">
            ...
            <p className="text-sm text-slate-500">Seu usuário não possui permissão para acessar nenhum setor do sistema no momento. Contate o administrador.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 justify-center">
            {modulosVisiveis.map((mod) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-14 h-14 rounded-2xl border ${mod.corIcone} flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300 shadow-sm`}>
                    <Icone size={26} strokeWidth={2.2} />
                  </div>
                  
                  <h3 className="text-lg font-bold text-slate-800 mb-2 leading-tight group-hover:text-blue-600 transition-colors">{mod.nome}</h3>
                  <p className="text-xs text-slate-500 leading-relaxed mb-6 flex-1 font-medium">
                    {mod.descricao}
                  </p>
                  
                  <div className="mt-auto flex items-center text-xs font-bold text-slate-400 group-hover:text-blue-700 transition-colors">
                    Acessar painel <ArrowRight size={14} className="ml-2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
                  </div>
                </button>
              )
            })}
      ...
      <div className="mt-auto py-4 text-center relative z-10 opacity-70">
        <div className="flex items-center justify-center gap-2 text-slate-500 text-[10px] font-bold tracking-[0.2em] uppercase">
